(function($){
	$.cometChat = {
		...
		clientWithWindow:false,
		clientPeerUser:null,
		clientPeerDiscMsg:false,
		...
		send:function(textMessage, peerUserName) {
            ...
            $.cometd.publish('/service/privatechat', {
                room: '/chat/client',//room: '/chat/demo',
        ...
        members:function(message) {
            ...
            $.each(message.data, function() {
	            if($.cometChat.loginUserName.includes("Emp_")){//si es empleado le muestra la lista
	            	if(this.includes("Emp_")){//si es empleado lo agrega a la lista	
	            ...
	            } else if ($.cometChat.loginUserName.includes("Cli_") || $.cometChat.loginUserName.includes("Unk_")){
	            	if(this.includes("Emp_")){	            	
	            		emp[emp.length]=this;	            		
	            	}
	            	
	            	if($.cometChat.clientPeerUser!=null){
		            	if($.cometChat.clientPeerUser.trim() == this.trim()){
		            		isClientPeerOnline=true;
		            		console.log($.cometChat.clientPeerUser +'- '+ this+'    '+isClientPeerOnline);
		            	}
	            	}
	            }
            });
          
            if (($.cometChat.loginUserName.includes("Cli_") || $.cometChat.loginUserName.includes("Unk_")) && $.cometChat.clientWithWindow==false){
            	emp=shuffle(emp);
            	if(emp.length>0){
            		$.cometChat.clientWithWindow=true;
            		isClientPeerOnline=true;
            		$.cometChat.clientPeerUser=emp[0];
            		createWindow($.cometChat.loginUserName,$.cometChat.clientPeerUser);
            	}
            }
            if (($.cometChat.loginUserName.includes("Cli_") || $.cometChat.loginUserName.includes("Unk_")) && $.cometChat.clientWithWindow==true && isClientPeerOnline==false && $.cometChat.clientPeerDiscMsg==false){
            	console.log($.cometChat.clientPeerUser +'    '+isClientPeerOnline);
            	$.cometChat.clientPeerDiscMsg=true;
            	var chatReceivingWindowDis = createWindow($.cometChat.loginUserName, $.cometChat.clientPeerUser);
            	chatReceivingWindowDis.appendMessage($.cometChat.clientPeerUser, 'El usuario se ha desconectado.', $.cometChat.loginUserName);
            }else if (($.cometChat.loginUserName.includes("Cli_") || $.cometChat.loginUserName.includes("Unk_")) && $.cometChat.clientWithWindow==true && isClientPeerOnline==true && $.cometChat.clientPeerDiscMsg==true){
            	$.cometChat.clientPeerDiscMsg=false;
            	var chatReceivingWindowDis = createWindow($.cometChat.loginUserName, $.cometChat.clientPeerUser);
            	chatReceivingWindowDis.appendMessage($.cometChat.clientPeerUser, 'El usuario se ha conectado.', $.cometChat.loginUserName);
            }
...
//			$.cometChat._membersSubscription = $.cometd.subscribe('/members/demo', $.cometChat.members); //channel handling members.
        	$.cometChat._chatSubscription    = $.cometd.subscribe('/chat/client',    $.cometChat.receive); //channel handling chat messages
			$.cometChat._membersSubscription = $.cometd.subscribe('/members/client', $.cometChat.members); //channel handling members.
        },
